...
pub(crate) enum PullAction {
    ...
    FreshClone,
    /// Checkout exists; attempt incremental update (pull+reset+resync sparse paths).
    Update,
...
impl PullPlan {
    pub(crate) fn decide(checkout_dir: &Path, tracked: &TrackedFiles) -> PullAction {
        if !checkout_dir.exists() {
            return PullAction::FreshClone;
        }
