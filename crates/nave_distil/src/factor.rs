...
pub(crate) fn find_factors(
    ...
    while !remaining.is_empty() {
        ...
        while i < remaining.len() {
            ...
            let within_scale = candidate_size >= min_allowed
                && candidate_size <= max_in_group;
...
fn prune_subsets(
    ...
    for i in 0..entries.len() {
        ...
        for j in 0..entries.len() {
            ...
            if entries[i].1.is_subset(entries[j].1)
                && entries[i].1.len() < entries[j].1.len()
            {
                keep[i] = false;
                break;
            }
    ...
    entries
        .into_iter()
        .zip(keep)
        .filter_map(|((k, v), keep)| if keep { Some((k.clone(), v.clone())) } else { None })
        .collect()
}
...
pub(crate) fn build_factored_report(
    ...
    for (member_name, indices) in factor_members.iter().zip(member_instances.iter()) {
        ...
        let report = build_group_from_values(
            "(cohort)",
            &cohort_instances,
            &cohort_values,
        );
